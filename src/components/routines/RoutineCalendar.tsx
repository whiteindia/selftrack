  ...
  const groupRoutinesByTimeSlot = (routines: Routine[]) => {
    ...
    routines.forEach(routine => {
      if (routine.preferred_slot_start && routine.preferred_slot_end) {
        ...
        const endHour = parseInt(routine.preferred_slot_end.split(':')[0]);
        
        // Handle routines that span multiple hours
        let currentHour = startHour;
        while (true) {
          const slotKey = currentHour.toString();
          ...
          // Move to next hour, handling day wraparound
          currentHour = (currentHour + 1) % 24;
          
          // Safety check to prevent infinite loop
          if (currentHour === startHour) break;
        }
        
        routinesWithSlots.push(routine);
      } else if (routine.preferred_slot_start) {
        // Handle routines with only start time (single hour)
        const startHour = parseInt(routine.preferred_slot_start.split(':')[0]);
