...
import { Plus, Edit, Trash2, Filter, X, StickyNote, ChevronDown, ChevronUp, Search, Tag, Code } from 'lucide-react';
...
const CodiNotes = () => {
  ...
  const createTagMutation = useMutation({
    ...
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ['codi-tags'] });
      setNewTagName('');
      setNewTagColor('#6366f1');
      setIsCreateTagDialogOpen(false);
      toast.success('Codi tag created successfully!');
    },
    onError: (error) => {
      toast.error('Failed to create codi tag: ' + error.message);
    }
  });
  ...
  const createNoteMutation = useMutation({
    ...
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ['codi-notes'] });
      setNewNote({
        title: '',
        content: '',
        service_id: '',
      ...
      toast.success('Coding note created successfully!');
    },
    onError: (error) => {
      toast.error('Failed to create coding note: ' + error.message);
    }
  });
  ...
  const updateNoteMutation = useMutation({
    ...
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ['codi-notes'] });
      setEditingNote(null);
      toast.success('Coding note updated successfully!');
    },
    onError: (error) => {
      toast.error('Failed to update coding note: ' + error.message);
    }
  });
  ...
  const deleteNoteMutation = useMutation({
    ...
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ['codi-notes'] });
      toast.success('Coding note deleted successfully!');
    },
    onError: (error) => {
      toast.error('Failed to delete coding note: ' + error.message);
    }
  });
      ...
      <Navigation>
        <div className="container mx-auto px-4 py-8">
          <div className="flex items-center justify-center h-64">
            <div className="text-gray-500">Loading coding notes...</div>
          </div>
        </div>
      </Navigation>
    );
      ...
      <div className="container mx-auto px-4 py-8">
        <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 mb-6">
          <div>
            ...
            <p className="text-gray-600 mt-1">Manage your coding notes and snippets</p>
          </div>
        ...
        {filteredNotes.length === 0 && (
          <div className="text-center py-12">
            ...
            <h3 className="text-lg font-medium text-gray-900 mb-2">No coding notes found</h3>
            <p className="text-gray-500 mb-4">
              {searchTerm || serviceFilter !== 'all' || clientFilter !== 'all' || projectFilter !== 'all' || selectedTagFilters.length > 0
                ? 'Try adjusting your filters to see more results.'
                : 'Create your first coding note to get started.'}
            </p>
            {!searchTerm && serviceFilter === 'all' && clientFilter === 'all' && projectFilter === 'all' && selectedTagFilters.length === 0 && (
              <Button onClick={() => setIsCreateDialogOpen(true)}>
                <Plus className="h-4 w-4 mr-2" />
                Create First Coding Note
              </Button>
            )}
          </div>
        )}
        ...
        <Dialog open={isCreateDialogOpen} onOpenChange={setIsCreateDialogOpen}>
          <DialogContent className="max-w-2xl max-h-[90vh] overflow-y-auto">
            <DialogHeader>
              <DialogTitle>Create New Coding Note</DialogTitle>
              <DialogDescription>
                Add a new coding note with title, content, and optional tags.
              </DialogDescription>
            </DialogHeader>
            ...
            <div className="space-y-4">
              <div>
                ...
                <Input
                  ...
                  placeholder="Enter coding note title..."
                />
              </div>
              ...
              <div>
                ...
                <Textarea
                  ...
                  placeholder="Enter your coding notes, snippets, or explanations..."
                  rows={8}
                />
              </div>
            ...
            <div className="flex justify-end gap-2">
              ...
              <Button onClick={handleCreateNote} disabled={createNoteMutation.isPending}>
                {createNoteMutation.isPending ? 'Creating...' : 'Create Coding Note'}
              </Button>
            </div>
          </DialogContent>
        </Dialog>
        ...
        {editingNote && (
          <Dialog open={!!editingNote} onOpenChange={() => setEditingNote(null)}>
            <DialogContent className="max-w-2xl max-h-[90vh] overflow-y-auto">
              <DialogHeader>
                <DialogTitle>Edit Coding Note</DialogTitle>
                <DialogDescription>
                  Update your coding note content and details.
                </DialogDescription>
              </DialogHeader>
        ...
        <Dialog open={isCreateTagDialogOpen} onOpenChange={setIsCreateTagDialogOpen}>
          <DialogContent>
            <DialogHeader>
              ...
              <DialogDescription>
                Add a new tag to organize your coding notes.
              </DialogDescription>
            </DialogHeader>
