...
const Citman = () => {
  ...
  // Get Citman-specific services and pre-fill them
  const citmanServices = useMemo(() => {
    const citmanKeywords = [
      'Citizen_Engg-Aptitude-GA-CA',
      'Citizen_Arnab-Charan-Modi',
      'Citzen_Social3-Atvist-Law-Philo-RAW-NGO-Hungr',
      'Citzen_Geography-Env-Earth Scientist'
    ];
    
    return allServices.filter(service => 
      service.name && citmanKeywords.some(keyword => service.name.includes(keyword))
    );
  }, [allServices]);

  // Set default services when services are loaded (Citman specific)
  React.useEffect(() => {
    if (citmanServices.length > 0 && selectedServices.length === 0) {
      const serviceNames = citmanServices.map(s => s.name);
      console.log('Pre-filling Citman services:', serviceNames);
      setSelectedServices(serviceNames);
    }
  }, [citmanServices, selectedServices.length]);

  // Pre-filter tasks based on Citman service criteria
  const citmanFilteredTasks = useMemo(() => {
    const citmanKeywords = [
      'Citizen_Engg-Aptitude-GA-CA',
      'Citizen_Arnab-Charan-Modi',
      'Citzen_Social3-Atvist-Law-Philo-RAW-NGO-Hungr',
    ...
    return allTasks.filter(task => 
      task.project_service && citmanKeywords.some(keyword => 
  ...
  const tasksForSelectedServices = useMemo(() => {
    if (selectedServices.length === 0) return citmanFilteredTasks;
    
    return citmanFilteredTasks.filter(task => 
      selectedServices.includes(task.project_service)
    );
  }, [citmanFilteredTasks, selectedServices]);

  // Derive available clients ONLY from tasks that match selected services
    ...
    <Navigation>
      <div className="space-y-6">
        <div className="flex justify-between items-center">
          <h1 className="text-3xl font-bold">Citman</h1>
          ...
          <Card>
            ...
            <CollapsibleContent>
              <CardContent className="space-y-4">
                {/* Service Filter - Show Citman-specific services */}
                <div>
                  <h3 className="text-sm font-medium mb-2">Services (Citman)</h3>
                  <div className="flex flex-wrap gap-2">
                    {citmanServices.map((service) => (
                      <Button
                        key={service.name}
