...
const Cman = () => {
  ...
  // Get Cman-specific services and pre-fill them
  const cmanServices = useMemo(() => {
    return allServices.filter(service => 
      service.name && service.name.includes('Cman_FRC')
    );
  }, [allServices]);

  // Set default services when services are loaded (Cman specific)
  React.useEffect(() => {
    if (cmanServices.length > 0 && selectedServices.length === 0) {
      const serviceNames = cmanServices.map(s => s.name);
      console.log('Pre-filling Cman services:', serviceNames);
      setSelectedServices(serviceNames);
    }
  }, [cmanServices, selectedServices.length]);

  // Pre-filter tasks based on Cman service criteria
  const cmanFilteredTasks = useMemo(() => {
    return allTasks.filter(task => 
      task.project_service && task.project_service.includes('Cman_FRC')
    );
  }, [allTasks]);

  // Filter tasks based on selected services
  const tasksForSelectedServices = useMemo(() => {
    if (selectedServices.length === 0) return cmanFilteredTasks;
    
    return cmanFilteredTasks.filter(task => 
      selectedServices.includes(task.project_service)
    );
  }, [cmanFilteredTasks, selectedServices]);
  ...
  // Filter tasks based on selected clients (from the service-filtered tasks)
  ...
  // Derive available projects ONLY from tasks that match selected services + clients
  ...
  // Final filtered tasks (this is what gets displayed)
          ...
          <Card>
            ...
            <CollapsibleContent>
              <CardContent className="space-y-4">
                {/* Service Filter - Show services containing Cman_FRC */}
                <div>
                  <h3 className="text-sm font-medium mb-2">Services (Cman_FRC)</h3>
                  <div className="flex flex-wrap gap-2">
                    {cmanServices.map((service) => (
                      <Button
                        key={service.name}
