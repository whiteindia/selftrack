...
-- First, let's check the current RLS policy for tasks and update it
-- The issue is that assignee_id and assigner_id are employee IDs, not auth user IDs
-- We need to join through the employees table to match the logged-in user
...
-- Create a function to get the current user's employee ID
...
-- Update the apply_rls_policies function to use the correct logic for tasks
...
BEGIN
  ...
  -- Apply RLS policies for tasks table - FIXED VERSION
  FOR policy_record IN 
  ...
  LOOP
    IF policy_record.rls_enabled THEN
      ...
      ELSE
        -- Other roles can only see tasks where they are assignee or assigner (using employee ID lookup)
  ...
  LOOP
    IF policy_record.rls_enabled THEN
      ...
      ELSE
        -- Other roles can only see sprints where they are the sprint leader (using employee ID lookup)
