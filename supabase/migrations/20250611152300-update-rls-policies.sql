...
-- Update the apply_rls_policies function to include tasks and sprints tables
...
BEGIN
  ...
  LOOP
    IF policy_record.rls_enabled THEN
      ...
      ELSE
        -- Other roles can only see tasks for projects of their client
        EXECUTE 'CREATE POLICY "' || policy_name || '" ON public.tasks FOR ALL USING (
          project_id IN (
            SELECT id FROM public.projects WHERE client_id = public.get_user_client_id(auth.uid())
          )
        )';
      END IF;
    END IF;
  ...
  LOOP
    IF policy_record.rls_enabled THEN
      ...
      ELSE
        -- Other roles can only see sprints that contain tasks for their client's projects
